...
export async function GET(req, context) {
    ...
    try {
        ...
        for (const c of data.rows) {
            // 1. Match frontend exactly: replace spaces with hyphens and force uppercase.
            // This preserves characters like '&', brackets, and periods.
            const name = c.companyname?.replace(/\s+/g, "-").toUpperCase() || "UNKNOWN";

            const cin = encodeURIComponent(c.cin || "");
            const state = encodeURIComponent(c.companystatecode?.toLowerCase().replace(/\s+/g, "_") || "unknown");

            // 2. Build the literal URL string
            const rawPath = `${BASE_URL}/${name}/${cin}/india/${state}/company-business-financial-credit-report`;

            // 3. Escape for XML syntax safety (converts raw '&' to '&amp;' so the XML parser doesn't crash)
            // Googlebot reads this as the true literal '&' path in the address bar.
            const xmlSafePath = rawPath.replace(/&/g, "&amp;");

            xml += `<url><loc>${xmlSafePath}</loc></url>\n`;
        }
