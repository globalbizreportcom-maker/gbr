...
export async function GET(req, { params }) {
    ...
    const { stateSitemap } = resolvedParams;
    const LIMIT = 100;

    // Next.js catch-all routes ([...slug]) return an Array.
    // If it's an array, extract the first value. If it's empty, default to empty string.
    const sitemapString = Array.isArray(stateSitemap)
        ? (stateSitemap[0] || "")
        : (stateSitemap || "");

    // Validate that the extracted string matches the pattern safely
    if (!sitemapString || !sitemapString.startsWith('sitemap-') || !sitemapString.endsWith('.xml')) {
        return new Response('Not Found', { status: 404 });
    }

    // Now it is safe to process as a string!
    const stateSlug = sitemapString.replace('sitemap-', '').replace('.xml', '');
